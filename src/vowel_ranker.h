/*
Header for rankers related to penalizing consonants.

This file includes VowelRanker and ProgressVowelRanker.
...
// Set of vowels used by rankers (initialized here because too
...
This class derives from AbstractRanker and will penalize the presence
of consonants when ranking a word. A user can control how many letters
...
one may want to be more lenient on the solver choosing words with
consonants. This class operates on an AbstractRanker that will
actually rank the word (e.g. LetterRanker or SubstringRanker). To
better understand the relationship of ProgressVowelRanker and the
AbstractRanker it uses, see the comments of the member functions
below.
*/
class ProgressVowelRanker : public AbstractRanker
{
public:
    /*
    Constructs a ProgressVowelRanker where consonants are always penalized.

    Parameters:
        r: Pointer to AbstractRanker that will be used by this class.
        cp: Penalty applied for each consontant letter.
    */
    ProgressVowelRanker(AbstractRanker *r, int cp);

    /*
    Overloaded version of ProgressVowelRanker(AbstractRanker*,int) that limits when consonants are penalized.
    ...
    */
    ProgressVowelRanker(AbstractRanker *r, int cp, unsigned short nf);

    /*
    Overloaded version of ProgressVowelRanker(AbstractRanker*,int) that allows name specification.
    ...
    Parameters:
        ...
        cp: Penalty applied for each consonant.
    */
    ProgressVowelRanker(std::string_view name, AbstractRanker *r, int cp);

    /*
    Overloaded version of ProgressVowelRanker(AbstractRanker*,int,unsigned short) that allows name specification.
    ...
    Parameters:
        ...
        nf: Number of letters that need to be found for duplicates to no longer be penalized.
    */
    ProgressVowelRanker(std::string_view name, AbstractRanker *r, int cp, unsigned short nf);
